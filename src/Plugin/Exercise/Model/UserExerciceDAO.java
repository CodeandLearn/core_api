...
public class UserExerciceDAO extends Model {
    protected void setGet(String request) {
        ...
        for (HashMap<String, Object> aResult : sql.getResultSet()) {
            ...
            userExerciseObj.exercise_id = (Integer) aResult.get("exercise_id");
    ...
    public UserExerciceDAO update(String socket, int id, JSONObject jsonObject) {
        ...
        make.add(jsonObject.getString("exercice_id"));
        make.add(id);
        setPut(socket, SQL.make("UPDATE user_exercises SET account_id=?, exercice_id=? WHERE id=?", make.toArray()));
        return this;
    }

...
/**
 * Created by teddy on 24/03/2016.
 */
public class UserGetId extends Default {
    private int id;
    ...
    public UserGetId(HttpServletRequest request, HttpServletResponse reply, int id) {
        super(request, reply);
        this.id = id;
        sqlUserId();
    }
    ...
    public void sqlUserId() {
        SQLiteJDBC sql = new SQLiteJDBC("db_SQLlite");
        ResultSet result = sql.selectDB("SELECT * FROM accounts WHERE id=" + id);
        try {
            if (result.next()) {
                id = result.getInt("id");
